...
- **余白効果**: 命令せずに問いかけや余韻で終わると、読者が自然と反応したくなる

### 構成
1. **共感フック（1〜2行）**: 読者の内面状態を言語化。「言いたかったけど言えなかった感情」を代弁する
2. **龍脈命術的な視点（2〜3行）**: なぜその状態が起きているか、方位・気の観点から短く説明
3. **締め（1行）**: 問いかけか余韻。命令形を避ける

### 使える締めの型（命令形NG、問いかけ・余韻OK）
- 共感系: 「これ、わかる？」「あなたもそう感じてた？」「心当たりある？」
- 発見系: 「知らなかった人、いる？」「これ、見落としてた？」
- 状況確認: 「今の自分、どっちかな」「心当たり、ある？」
...
1. **フック（1行）**: 読者の内言または状況を一言で突く（「先日、〜の方を鑑定しました」はその後に）
2. **悩み（1〜2行）**: 具体的な苦しさを逆接で描写。詳しく書きすぎない
3. **鑑定の発見（1行）**: 「原因は〇〇でした」と一言で明かす
4. **変化（1行）**: 行動と結果を一文で
5. **メッセージ（1行）**: 「〇〇のせいじゃない。〇〇だっただけ」の形で締める
6. **CTA**: 「▷ 鑑定ページ {service_url}」
...
- **制限型の書き出しも有効（カリギュラ効果）**: 「今うまくいっている人には関係ない話。でも〜な感覚がある人だけ読んで」など
- 末尾は必ず「わかる人いる？」「これ、あなたのこと？」「○か✕でコメントして」など**コメントを促す問いかけ**で締める
- **100〜160文字**。短く刺さる一言を積み重ねる構成。説明しすぎない
...
"""
    ...
    # kyokan / engagement_hook / follow_cta など URL 未設定の conversion パターン向け
