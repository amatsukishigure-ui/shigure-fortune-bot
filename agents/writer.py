...
### 構成（50〜90字）
1. **問いかけ（1行）**: 「今の自分、どっちに近い？」「正直に教えて」など
2. **選択肢（2〜3択）**: 各選択肢を「状況の説明 → 絵文字」の形式で
3. **コメント促進（1行）**: 「絵文字だけでいいからコメントして」
4. **個別返信の予告（1行）**: 「後で個別に返信するね」← 必須

### 選択肢のテーマ例（テーマに合わせて自由に変える）
- 気の状態: 「整ってきた → 🔮」vs「詰まってる → 🐉」
- 行動意欲: 「動きたい → ✨」vs「止まってる → 🌙」
- 悩みの種類: 「仕事・キャリア → 🔥」vs「恋愛・人間関係 → 💧」vs「お金 → 💰」
