...
"""
        ...
        from core.kaiun_calendar import _load_cache, get_moon_phase
        cache = _load_cache()
        # 月相データを取得（外部ライブラリ不要の純粋計算）
        _moon = get_moon_phase(
            __import__('datetime').date.fromisoformat(est_date_str) if est_date_str else None
        )
        _moon_block = (
            f"\n【今日の月相】{_moon['phase_emoji']} {_moon['phase_name']}"
            f"（気のエネルギー: {_moon['energy']}）"
            f" / 次の{_moon['next_event']}まであと{_moon['days_to_next']}日"
        )
...
- **月相情報も必ず投稿に組み込むこと**（例: 「満月は完成・解放のエネルギー」「新月はリセットと始まり」）

### 必須: 行動許可型で書く（最重要）
実績データ:
- 行動許可型（「この大安一粒万倍日に、今日こそやってみて」）→ eng=45（最高値）
- 吉日説明型（「一粒万倍日とは〇〇の日で〜」）→ eng=3（最低値）

❌ 禁止: 「一粒万倍日とは…」「大安は六曜の中で最も吉の日とされ…」（説明で終わる）
✅ 必須: 「この{day_label}に動く許可を出す」型で書く

行動許可型の構成（150〜220字）:
1. 冒頭（1行）: 「今日は〇〇の日。ずっと迷っていたこと、動いていい。」「この{day_label}に動く許可が出た。」
2. 具体アクション（1〜2行）: 「方位は〇〇。朝のうちに〇〇するだけでいい。」「財布の整理だけでも。」など
3. 時限感（1行）: 「次の同じ開運日は〇日後。」「この組み合わせは今日だけ。」
4. 締め（1行）: 「動いた人 ✨ をコメントに」or「保存して今日使って」

- 語尾はカジュアルでOK（「〜してみて」「〜がいいよ」）

### 心理効果
- **損失回避**: 「次は〇日後」の表現が行動を後押しする
- **行動許可**: 「動いていい」「許可が出た」という表現が迷っている人の背中を押す
"""
...
- **冒頭スクロール停止フック（1行）**: 「♈ 牡羊座 →」で絶対に始めない。
  ...
  → これらはbrand_ryumyaku/jisseki_ninshouに任せる
...
### 重要：出力形式
以下の形式で4つの投稿テキストをJSON配列として出力すること：
["1投稿目", "2投稿目", "3投稿目", "4投稿目"]

### 最重要ルール：「引き」は内容ヒット型のみ
各投稿（4投稿目以外）の末尾は「次に何が来るかを匂わせる一文」で締める。
