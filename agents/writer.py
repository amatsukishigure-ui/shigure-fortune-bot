...
"""
        ...
        # 1〜3位星座をランダム化（同じ顔ぶれが上位に固定されるのを防ぐ）
        _kr_top_candidates = [
            ("いて座",   "「宿命の向きが整ったとき、気が一気に動く」"),
            ("いて座",   "「遠くへ向かう気が、今の流れを一変させる」"),
            ("みずがめ座","「独自の向きに気を整えると、流れが変わる」"),
            ("みずがめ座","「鏡の向きを整えると、気の反射が変わる」"),
            ("てんびん座","「受け取る準備が整ったとき、気が動き始める」"),
            ("てんびん座","「バランスの向きが合った瞬間、気が安定する」"),
            ("さそり座", "「深く潜った分だけ、気の力が強まる」"),
            ("さそり座", "「本質に向かうとき、方位の力が最大化する」"),
        ...
        _kr_pool = list(_kr_top_candidates)
        _rnd_kr.shuffle(_kr_pool)
        _kr_top1_zodiac, _kr_top1_phrase = _kr_pool[0]
        _kr_top2_zodiac, _kr_top2_phrase = _kr_pool[1]
        _kr_top3_zodiac, _kr_top3_phrase = _kr_pool[2]
        pattern_extra = f"""
## 龍脈言霊ランキング型の追加ルール
...
3. ランキング本体（全12星座を必ず全部含める）: 🥇🥈🥉🏅を1〜4位に使い、以下は「第◯位」
   ...
   - **今回の1位**: {_kr_top1_zodiac} → 🥇第1位　{_kr_top1_zodiac}{_kr_top1_phrase}
   - **今回の2位**: {_kr_top2_zodiac} → 🥈第2位　{_kr_top2_zodiac}{_kr_top2_phrase}
   - **今回の3位**: {_kr_top3_zodiac} → 🥉第3位　{_kr_top3_zodiac}{_kr_top3_phrase}
   - 4〜12位は残り9星座を龍脈命術的根拠でランク付けする
4. 締めemoji CTA（2〜3行）:
{_kr_cta}
...
"""
        ...
        # 手放しテーマは実績eng=102（最高）→ 3倍の出現確率で重点投入
        _combo_themes = {
            "手放し_a": {
            ...
            "手放し_b": {
                "intro": "手放せた分だけ、気の流れが変わり始める。",
                "rank_label": "今、手放しの気が最も届きやすい",
                "closing": "龍脈命術では、手放しは「空間を作ること」——器が空いた人から、次が流れ込む。",
                "cta_emoji": "🌿",
                "cta_text": "を置いた人から、溜まっていた重さが少しずつ流れていきますように",
            },
            "手放し_c": {
                "intro": "止まっている気は、手放しで動き始める。",
                "rank_label": "手放しで流れが変わる気が最も強まる",
                "closing": "方位の気は「詰まった場所」には流れない。手放した瞬間から、新しい流れが入ってくる。",
                "cta_emoji": "🌙",
                "cta_text": "を置いた人から、つかえていたものが静かに溶け始めますように🌌",
            },
            "金運": {
        ...
        # 手放し3テーマに3倍の重みをかけた選択
        _combo_keys_weighted = (
            ["手放し_a", "手放し_a", "手放し_b", "手放し_b", "手放し_c", "手放し_c",
             "金運", "縁・恋愛", "天職"]
        )
        _theme_key = _rnd_combo.choice(_combo_keys_weighted)
        _th = _combo_themes[_theme_key]
        _theme_display = _theme_key.split("_")[0]  # "手放し_a" → "手放し"

        # 季節的・感情的フックを冒頭に加えるオプション（top投稿「嵐の中で〜」の再現）
        _seasonal_hooks = [
            "今、何かを溜め込んでいませんか。",
            "最近、気の流れが滞っていると感じていませんか。",
            "「頑張っているのに動けない」——それは気の詰まりかもしれません。",
            "",  # hookなし（30%の確率）
            "",
            "",
        ]
        _hook = _rnd_combo.choice(_seasonal_hooks) if _theme_display == "手放し" else ""
        _hook_line = f"\n{_hook}\n" if _hook else "\n"
...
### テーマ: {_theme_display}
### 感情フック（冒頭任意追加）: {_hook if _hook else "なし（introから直接始める）"}

### 構成（厳守）
1. 感情フック（任意1行）: {_hook if _hook else "省略してintroから始める"}
2. テーマ導入（1行）: 「{_th['intro']}」の形式で書く。**先頭または2行目までに「星座」または「♈〜♓」などの星座記号を必ず含めること。**
3. ランキングタイトル（1行）: 「{_th['rank_label']} 生まれ月×星座 top5〜8」
4. ランキング本体（5〜8項目・改行区切り）:
   形式: 「1位 〇月生まれ × △△座」
   - 月は1〜12月の中から選ぶ（重複なし）
   - 星座は12星座から選ぶ（重複なし）
   - 龍脈命術的な根拠（月の気×星座の特性の相性）で組み合わせる
5. 補足1行: 「{_th['closing']}」
6. 絵文字CTA（1〜2行）: 「『{_th['cta_emoji']}』{_th['cta_text']}」

### 実績TOP投稿の勝ちパターン（必ず参考）
eng=102: 「嵐の中で「動こう」としていませんか。」→「手放しの流れが今月最も強く届く 生まれ月×星座 top7」→「「⭐」を置いた人から、抱えてきた重さが少しずつ抜けていきますように🌌」
→ 冒頭の問いかけ＋感情共鳴フック＋祈願CTAが高eng三要素
...
def run(batch_size: int = 5) -> dict:
    ...
    for i in range(batch_size):
        ...
        if is_thread_pattern:
            ...
            _HIGH_ENG_PIDS = {"kotodama_rank", "caligula", "lucky_combo", "zodiac_ranking", "fuusui_tip"}
