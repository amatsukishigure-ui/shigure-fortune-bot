...
def _select_pattern(
    ...
    for p in available:
        ...
        # ブランドサイクル曜日ブースト
        # 火曜（wd=1）: brand_ryumyaku でブランド認知を週次強化
        if _today_wd == 1 and pid == "brand_ryumyaku":
            w *= 2.0
        # 木曜（wd=3）: jisseki_ninshou で信頼・実績を週次強化
        if _today_wd == 3 and pid == "jisseki_ninshou":
            w *= 2.0
        # 土曜（wd=5）: world_view で哲学・起源ストーリーを週次強化
        if _today_wd == 5 and pid == "world_view":
            w *= 2.0
...
### 役割（最重要）
**占い知識ゼロ・懐疑的な人への入口コンテンツ**として機能させる。
「占いっぽくない」入口から入り、「でも龍脈命術ではこういう理由があるんだ」と気づかせる。

### ichi_gonとの違い
- ichi_gon = 占術的断言・名言風・抽象的（既存フォロワー向け）
- pop_short = **日常の具体的ヒント**（占いを知らない人でも「試せる」と感じる）

### トピック例（ローテーションして使う）
- 財布・お金の置き場所 → 「床に置くと気が下がる」
- 玄関・靴 → 「靴がそろっていると気の入り口が開く」
- デスク・仕事スペース → 「北向きに座ると集中力の気が安定する」
- 寝室・枕の向き → 「頭を北に向けると深眠りの気が入りやすい」
- スマホ・通知 → 「通知オフにするだけでエネルギーの浪費が減る」
- 植物・観葉植物 → 「東の窓際に置くと気の浄化が進む」
- 窓・換気 → 「朝5分換気するだけで古い気が入れ替わる」
- キッチン・食器 → 「欠けた食器は金運の気を欠かす」

### 書き方のルール
- 2〜4行（80〜130文字）
- 語尾はカジュアルに（「〜だよ」「〜よね」「〜してみて」「〜かも」）
- 絵文字を1〜2個使ってOK
- **「占い」「吉方位」という言葉を使わなくてもOK**。「気」「方位」「龍脈命術では〜」を1行添えれば十分
- **NG→改善例の対比**: 「NG：財布を床に置く → 改善：バッグの中に定位置を作るだけ」
- **「〜するだけでいい」で行動の敷居を下げる**

### 心理効果
- **実行可能性効果**: 「今すぐできる」小さな行動提案が「試してみよう」を引き出す
- **間口の広さ**: 占い知識ゼロの人が最初にたどり着けるコンテンツ
- **親近感効果**: カジュアルな語尾が距離感を縮め、フォローしたくなる人格を演出する
"""
...
**⑤ 懐疑層向け型（新規層に最も刺さる）**: 「占いを信じていない人にこそ読んでほしい」
  例「「占いなんて信じない」という人に、一度だけ聞いてほしいことがある。」
  例「信じる・信じないは関係ない、という話をしてもいい？」
  例「占術家がこんなことを言うのも変だけど、「信じなくていい」と思っている。」
  → この型は**新規フォロワー獲得に最も効果的**。疑念を先取りすることで抵抗感が消える
...
"""
        ...
        import random as _rnd_wv
        _wv_mode = _rnd_wv.choice(["A", "B"])
        if _wv_mode == "A":
            pattern_extra = """
## 世界観投稿型 — Aモード（哲学語り）

### 構成
- 読者が心の中で思っている疑問・反論を冒頭で受け止め、時雨としての答えを語る
- 「龍脈命術」という固有名詞を必ず1回使い、「時雨の独自メソッド」という印象を残す
- 末尾に「占術家 時雨」のシグネチャ
- **100〜160文字**（シグネチャ含む）。余白を意識して、語りすぎない

### 冒頭パターン例（読者の内言を引用する形が効果的）
- 「「占いって結局気休めでしょ？」そう思っている人にだけ、正直に話します。」
- 「「努力すれば方位は関係ない」という声を聞くたびに、思うことがある。」
- 「占いを信じていない人が、一番助かることがある。」

### 禁止
- 自分語りが長くなる（シグネチャ含め160字以内）
- 読者が共感できない抽象論

### 心理効果
- **自己開示効果**: 時雨の本音・信念を語ることで人間らしさが生まれ、信頼と親近感が高まる
- **余白効果**: 語りすぎない部分が「もっと知りたい」というフォロー動機を生む
"""
        else:  # B: 起源ストーリー
            pattern_extra = """
## 世界観投稿型 — Bモード（起源・時雨の物語）

### 設計原則
「なぜ時雨が龍脈命術を使うのか」を1人称で語る。占術家としての原体験・転機・気づきを
コンパクトに届けることで、フォロワーが「この人を信頼したい」と感じるファン化を促す。

### 構成
1. **きっかけ（1行）**: 「最初に方位の力を実感したのは〜」「龍脈命術に出会う前は〜」など
2. **核心の気づき（2〜3行）**: 「当てる占いじゃなく、動かす占いが必要だと気づいた」
3. **今の時雨（1行）**: この道を続ける理由・読者への想い
4. **シグネチャ**: 末尾に「占術家 時雨」
...
### 文字数・トーン
- **110〜170文字**（シグネチャ含む）
- 語り口は静かで確信がある。過去の経験を淡々と、でも深く語る

### 禁止
- 自慢・誇張（「私はすごい」にならないよう自己開示にとどめる）
- 「感動した！」などの過剰な感情表現
"""
    elif pattern_id == "brand_ryumyaku":
        import random as _rnd_br
        _br_angle = _rnd_br.choice([
            "differentiation",   # 他の占いとの違い
            "mechanism",         # 方位・気が影響する仕組み
            "stagnation",        # 停滞の読み方
            "transition",        # 転機のサイン
            "honest_limits",     # できること・できないこと（誠実さ）
            "results",           # 動いた人に共通すること（実績角度）
        ])
        _br_angle_guide = {
            "differentiation": "「星座・タロット・数秘は『あなたの性質』を読む。龍脈命術は『あなたが今いる場所の気の流れ』を読む」——この対比を核心に。自分を変えるより向きを変える占い。",
            "mechanism":       "「同じ種でも植える土が違えば育ち方が変わる」の比喩。方位・場所の気が人の行動・縁・機会に直接影響する仕組みをわかりやすく。",
            "stagnation":      "「停滞は悪い時期じゃなく、方向を整えるタイミング」。努力してるのに動かない理由を龍脈命術の言葉で外在化する。",
            "transition":      "「もやもや・違和感・なんか今じゃない感覚は、気の流れが変わり始めるサイン」。転機を見逃さない読み方を伝える。",
            "honest_limits":   "「龍脈命術で変えられること・変えられないこと」を正直に語る。誠実さが信頼を高め、鑑定への信頼感につながる。",
            "results":         "「方位を変えた人に共通すること」——N人見てきた実感として、動いた人とそうでない人の決定的な違いを語る。",
        }[_br_angle]
        pattern_extra = f"""
## 龍脈命術ブランド型の追加ルール
- テーマ: {theme}
- 今回の切り口: **{_br_angle}**
- 切り口のガイド: {_br_angle_guide}
...
### 構成（120〜200字）
1行目: 「龍脈命術は〜」または「普通の占いと〜が違う」または切り口に応じた書き出し
中段: 核心の差別化・洞察ポイントを1〜2文
...
- 他の占いを「間違っている」と否定する
"""
    elif pattern_id == "jisseki_ninshou":
        import random as _rnd_jis
        _jis_type = _rnd_jis.choice(["volume", "pattern", "turning_point", "difference"])
        _jis_guide = {
            "volume":        "「N人鑑定してわかったこと」型。具体的な数字（1000人・500人など）で始め、鑑定を通じて見えてきた共通の法則を語る。",
            "pattern":       "「停滞している人に共通するパターン」型。何年も見てきた中で気づいた、うまくいかない人の共通点を龍脈命術の視点で語る。",
            "turning_point": "「転機が来た人に共通すること」型。動き出した人・変わった人に共通する行動・態度を語る。「準備した人が動けた」ではなく「向きを変えた人が変わった」という視点で。",
            "difference":    "「鑑定前と後で変わること」型。鑑定を受けた人が最も驚いた気づきや、よく言われる言葉を共有する。",
        }[_jis_type]
        pattern_extra = f"""
## 実績・信頼型の追加ルール
- 今回のアプローチ: **{_jis_type}**
...
### 設計原則
「時雨が何人見てきたか」「何年やってきたか」という経験の蓄積が信頼の根拠になる。
自慢ではなく「こういう人を何人も見てきた」というファクトの共有スタンスで書く。

### 構成（130〜190字）
1. **実績の枕（1行）**: 「1000人以上鑑定してわかったこと。」「何年も龍脈命術で人を見てきてわかること。」
2. **核心の洞察（2〜3行）**: 見てきた事実・共通パターンを断言形式で
3. **締め（1行）**: 「心当たりある？」「これ、当てはまる？」など問いかけで終わる

### 実績数字の使い方
- 「1000人以上」「何年も」「何百人」などの表現はOK
- 具体的すぎる数字（「1247人」など）は不自然なので使わない
- 「よく聞く」「何度も見てきた」という経験則表現も有効

### 禁止
- 自慢・誇大表現（「すごい実績」「業界最高」など）
- 根拠のない断言（「必ず変わる」「100%」）
- 鑑定URLを入れる（このパターンは信頼醸成が目的、直接CTAは避ける）
"""
...
- **【ブランド言語の統一】「龍脈命術」を必ず1回使う**: すべての投稿で「龍脈命術」という固有名詞を少なくとも1回使う。「占い」「風水」「吉方位」という汎用語だけでは「時雨の独自メソッド」という記憶が読者に残らない。「龍脈命術では〜」「龍脈命術の視点から〜」「龍脈命術で見ると〜」の形で自然に埋め込む
